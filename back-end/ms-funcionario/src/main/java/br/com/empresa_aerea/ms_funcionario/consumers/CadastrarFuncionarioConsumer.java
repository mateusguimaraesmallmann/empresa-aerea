...
public class CadastrarFuncionarioConsumer {
    ...
    @RabbitListener(queues = "ms-funcionario-cadastrar-funcionario")
    public void cadastrarFuncionario(Message message) throws JsonProcessingException {
        try {
            ObjectMapper objectMapper = new ObjectMapper();
            FuncionarioDTO funcionarioDTO = objectMapper.readValue(message.getBody(), FuncionarioDTO.class);

            FuncionarioResponseDTO funcionarioResponseDTO = service.cadastrarFuncionario(funcionarioDTO);
            ...
            rabbitTemplate.send(EXCHANGE_NAME, "saga-ms-funcionario-cadastrar-funcionario", responseMessage);
        } catch (Exception ex) {
